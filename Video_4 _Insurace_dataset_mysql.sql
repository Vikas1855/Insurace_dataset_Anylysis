...
select region,avg(claim) as avg_claim from insurance_data group by 1;
-- 3. Select the maximum and minimum BMI values in the table.
select * from insurance_data;
select max(bmi) as max_bmi,min(bmi) as min_bmi from insurance_data;

select max(bmi) as mx_bmi, min(bmi) as min_bmi,avg(bmi) as avg_bmi from insurance_data;
...
select region,count(PatientID)as patient from insurance_data where smoker="Yes"
group by region;

select region,count(PatientID) as patient, smoker from insurance_data where smoker = 'Yes'
group by region;
...
select PatientID,avg(claim) as avg_claim from insurance_data where diabetic ="Yes" and smoker = "Yes"
group by PatientID;
